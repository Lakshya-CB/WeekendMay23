...
public class BTree {
	...
	public void vertical() {
		HashMap<Integer, ArrayList<Integer>> HM = new TreeMap<>();
		
		Queue<Node> Q_n = new LinkedList<>();
		Queue<Integer> Q_a = new LinkedList<>();
		Q_n.add(root);
		Q_a.add(0);
		while(!Q_n.isEmpty()) {
			Node curr_n = Q_n.poll();
			int curr_a = Q_a.poll();
			
//			+++++++++++++++++
			ArrayList<Integer> AL = HM.getOrDefault(curr_a, new ArrayList<>());
			AL.add(curr_n.data);
			HM.put(curr_a, AL);
